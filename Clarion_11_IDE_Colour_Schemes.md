
# .NET System.Drawing.Color

https://learn.microsoft.com/en-us/dotnet/api/system.drawing.color?view=net-9.0

| .NET Name     | ARGB (A,R,G,B)       | Integer Value  | Hex Code   |
|---------------|----------------------|----------------|------------|
| AliceBlue     | (255,240,248,255)    | -984833        | #F0F8FF    |
| AntiqueWhite  | (255,250,235,215)    | -332841        | #FAEBD7    |
| Aqua          | (255,0,255,255)      | -16711681      | #00FFFF    |
| Aquamarine    | (255,127,255,212)    | -8388652       | #7FFFD4    |
| Azure         | (255,240,255,255)    | -983041        | #F0FFFF    |
| Beige         | (255,245,245,220)    | -657956        | #F5F5DC    |
| Bisque        | (255,255,228,196)    | -6972          | #FFE4C4    |
| Black         | (255,0,0,0)          | -16777216      | #000000    |
| BlanchedAlmond| (255,255,235,205)    | -5171          | #FFEBCD    |
| Blue          | (255,0,0,255)        | -16776961      | #0000FF    |
| BlueViolet    | (255,138,43,226)     | -7722014       | #8A2BE2    |
| Brown         | (255,165,42,42)      | -5952982       | #A52A2A    |
| BurlyWood     | (255,222,184,135)    | -2180985       | #DEB887    |
| CadetBlue     | (255,95,158,160)     | -10510688      | #5F9EA0    |
| Chartreuse    | (255,127,255,0)      | -8388864       | #7FFF00    |
| Chocolate     | (255,210,105,30)     | -2987746       | #D2691E    |
| Coral         | (255,255,127,80)     | -32944         | #FF7F50    |
| CornflowerBlue| (255,100,149,237)    | -10185235      | #6495ED    |
| Cornsilk      | (255,255,248,220)    | -1828          | #FFF8DC    |
| Crimson       | (255,220,20,60)      | -2354116       | #DC143C    |
| Cyan          | (255,0,255,255)      | -16711681      | #00FFFF    |
| DarkBlue      | (255,0,0,139)        | -16777077      | #00008B    |
| DarkCyan      | (255,0,139,139)      | -16741493      | #008B8B    |
| DarkGoldenrod | (255,184,134,11)     | -4687410       | #B8860B    |
| DarkGray      | (255,169,169,169)    | -5658199       | #A9A9A9    |
| DarkGreen     | (255,0,100,0)        | -16751616      | #006400    |
| DarkKhaki     | (255,189,183,107)    | -4343957       | #BDB76B    |
| DarkMagenta   | (255,139,0,139)      | -7667573       | #8B008B    |
| DarkOliveGreen| (255,85,107,47)      | -11179217      | #556B2F    |
| DarkOrange    | (255,255,140,0)      | -29696         | #FF8C00    |
| DarkOrchid    | (255,153,50,204)     | -6737204       | #9932CC    |
| DarkRed       | (255,139,0,0)        | -7667712       | #8B0000    |
| DarkSalmon    | (255,233,150,122)    | -1468806       | #E9967A    |
| DarkSeaGreen  | (255,143,188,143)    | -7357301       | #8FBC8F    |
| DarkSlateBlue | (255,72,61,139)      | -12042869      | #483D8B    |
| DarkSlateGray | (255,47,79,79)       | -13676721      | #2F4F4F    |
| DarkTurquoise | (255,0,206,209)      | -16724271      | #00CED1    |
| DarkViolet    | (255,148,0,211)      | -7077677       | #9400D3    |
| DeepPink      | (255,255,20,147)     | -10040065      | #FF1493    |
| DeepSkyBlue   | (255,0,191,255)      | -16728065      | #00BFFF    |
| DimGray       | (255,105,105,105)    | -11184811      | #696969    |
| DodgerBlue    | (255,30,144,255)     | -14774017      | #1E90FF    |
| Firebrick     | (255,178,34,34)      | -5103070       | #B22222    |
| FloralWhite   | (255,255,250,240)    | -1296          | #FFFAF0    |
| ForestGreen   | (255,34,139,34)      | -14513374      | #228B22    |
| Gainsboro     | (255,220,220,220)    | -2302756       | #DCDCDC    |
| GhostWhite    | (255,248,248,255)    | -460545        | #F8F8FF    |
| Gold          | (255,255,215,0)      | -10496         | #FFD700    |
| Goldenrod     | (255,218,165,32)     | -2448096       | #DAA520    |
| Gray          | (255,128,128,128)    | -8355712       | #808080    |
| Green         | (255,0,128,0)        | -16744448      | #008000    |
| GreenYellow   | (255,173,255,47)     | -5374161       | #ADFF2F    |
| Honeydew      | (255,240,255,240)    | -983056        | #F0FFF0    |
| HotPink       | (255,255,105,180)    | -38476         | #FF69B4    |
| IndianRed     | (255,205,92,92)      | -3318692       | #CD5C5C    |
| Indigo        | (255,75,0,130)       | -11861886      | #4B0082    |
| Ivory         | (255,255,255,240)    | -16            | #FFFFF0    |
| Khaki         | (255,240,230,140)    | -989556        | #F0E68C    |
| Lavender      | (255,230,230,250)    | -1644801       | #E6E6FA    |
| LavenderBlush | (255,255,240,245)    | -32513         | #FFF0F5    |
| LawnGreen     | (255,124,252,0)      | -8586240       | #7CFC00    |
| LemonChiffon  | (255,255,250,205)    | -1331          | #FFFACD    |
| LightBlue     | (255,173,216,230)    | -5383962       | #ADD8E6    |
| LightCoral    | (255,240,128,128)    | -1015680       | #F08080    |
| LightCyan     | (255,224,255,255)    | -2031617       | #E0FFFF    |
| LightGoldenrodYellow | (255,250,250,210) | -1120       | #FAFAD2    |
| LightGray     | (255,211,211,211)    | -2894893       | #D3D3D3    |
| LightGreen    | (255,144,238,144)    | -7278960       | #90EE90    |
| LightPink     | (255,255,182,193)    | -18751         | #FFB6C1    |
| LightSalmon   | (255,255,160,122)    | -24454         | #FFA07A    |
| LightSeaGreen | (255,32,178,170)     | -14634326      | #20B2AA    |
| LightSkyBlue  | (255,135,206,250)    | -7876870       | #87CEFA    |
| LightSlateGray| (255,119,136,153)    | -8943463       | #778899    |
| LightSteelBlue| (255,176,196,222)    | -5192482       | #B0C4DE    |
| LightYellow   | (255,255,255,224)    | -32            | #FFFFE0    |
| Lime          | (255,0,255,0)        | -16711936      | #00FF00    |
| LimeGreen     | (255,50,205,50)      | -13447886      | #32CD32    |
| Linen         | (255,250,240,230)    | -331546        | #FAF0E6    |
| Magenta       | (255,255,0,255)      | -65281         | #FF00FF    |
| Maroon        | (255,128,0,0)        | -8388608       | #800000    |
| MediumAquamarine | (255,102,205,170) | -10039894      | #66CDAA    |
| MediumBlue    | (255,0,0,205)        | -16777035      | #0000CD    |
| MediumOrchid  | (255,186,85,211)     | -4565541       | #BA55D3    |
| MediumPurple  | (255,147,112,219)    | -7059381       | #9370DB    |
| MediumSeaGreen| (255,60,179,113)     | -12799119      | #3CB371    |
| MediumSlateBlue| (255,123,104,238)   | -8689426       | #7B68EE    |
| MediumSpringGreen| (255,0,250,154)   | -16713062      | #00FA9A    |
| MediumTurquoise| (255,72,209,204)    | -10039894      | #48D1CC    |
| MediumVioletRed| (255,199,21,133)    | -3730043       | #C71585    |
| MidnightBlue  | (255,25,25,112)      | -15132304      | #191970    |
| MintCream     | (255,245,255,250)    | -655366        | #F5FFFA    |
| MistyRose     | (255,255,228,225)    | -6943          | #FFE4E1    |
| Moccasin      | (255,255,228,181)    | -6983          | #FFE4B5    |
| NavajoWhite   | (255,255,222,173)    | -8531          | #FFDEAD    |
| Navy          | (255,0,0,128)        | -16777088      | #000080    |
| OldLace       | (255,253,245,230)    | -133658        | #FDF5E6    |
| Olive         | (255,128,128,0)      | -8355840       | #808000    |
| OliveDrab     | (255,107,142,35)     | -9728477       | #6B8E23    |
| Orange        | (255,255,165,0)      | -23296         | #FFA500    |
| OrangeRed     | (255,255,69,0)       | -47872         | #FF4500    |
| Orchid        | (255,218,112,214)    | -2461482       | #DA70D6    |
| PaleGoldenrod | (255,238,232,170)    | -1249026       | #EEE8AA    |
| PaleGreen     | (255,152,251,152)    | -6943          | #98FB98    |
| PaleTurquoise | (255,175,238,238)    | -4927330       | #AFEEEE    |
| PaleVioletRed | (255,219,112,147)    | -2894893       | #DB7093    |
| PapayaWhip    | (255,255,239,213)    | -1041          | #FFEFD5    |
| PeachPuff     | (255,255,218,185)    | -9543          | #FFDAB9    |
| Peru          | (255,205,133,63)     | -3308225       | #CD853F    |
| Pink          | (255,255,192,203)    | -2039584       | #FFC0CB    |
| Plum          | (255,221,160,221)    | -2130703       | #DDA0DD    |
| PowderBlue    | (255,176,224,230)    | -5185306       | #B0E0E6    |
| Purple        | (255,128,0,128)      | -8388480       | #800080    |
| Red           | (255,255,0,0)        | -65536         | #FF0000    |
| RosyBrown     | (255,188,143,143)    | -4419697       | #BC8F8F    |
| RoyalBlue     | (255,65,105,225)     | -12490271      | #4169E1    |
| SaddleBrown   | (255,139,69,19)      | -7650029       | #8B4513    |
| Salmon        | (255,250,128,114)    | -360334        | #FA8072    |
| SandyBrown    | (255,244,164,96)     | -744352        | #F4A460    |
| SeaGreen      | (255,46,139,87)      | -13726889      | #2E8B57    |
| SeaShell      | (255,255,245,238)    | -1040          | #FFF5EE    |
| Sienna        | (255,160,82,45)      | -6270411       | #A0522D    |
| Silver        | (255,192,192,192)    | -4144960       | #C0C0C0    |
| SkyBlue       | (255,135,206,235)    | -7876885       | #87CEEB    |
| SlateBlue     | (255,106,90,205)     | -12042875      | #6A5ACD    |
| SlateGray     | (255,112,128,144)    | -8943463       | #708090    |
| Snow          | (255,255,250,250)    | -1286          | #FFFAFA    |
| SpringGreen   | (255,0,255,127)      | -16711809      | #00FF7F    |
| SteelBlue     | (255,70,130,180)     | -12156236      | #4682B4    |
| Tan           | (255,210,180,140)    | -2968436       | #D2B48C    |
| Teal          | (255,0,128,128)      | -16744320      | #008080    |
| Thistle       | (255,216,191,216)    | -2572328       | #D8BFD8    |
| Tomato        | (255,255,99,71)      | -40121         | #FF6347    |
| Turquoise     | (255,64,224,208)     | -12525360      | #40E0D0    |
| Violet        | (255,238,130,238)    | -1146130       | #EE82EE    |
| Wheat         | (255,245,222,179)    | -663885        | #F5DEB3    |
| White         | (255,255,255,255)    | -1             | #FFFFFF    |
| WhiteSmoke    | (255,245,245,245)    | -657931        | #F5F5F5    |
| Yellow        | (255,255,255,0)      | -256           | #FFFF00    |
| YellowGreen   | (255,154,205,50)     | -6632146       | #9ACD32    |


# .NET System.Drawing.SystemColors / Win32 API System Colours

https://learn.microsoft.com/en-us/dotnet/api/system.drawing.systemcolors?view=net-9.0

| .NET Name               | Description                                                                 | ARGB (A,R,G,B)       | Integer Value  | Hex Code   |
|------------------------|-----------------------------------------------------------------------------|----------------------|----------------|------------|
| ActiveBorder           | Color of the active window's border.                                       | (255,180,180,180)    | -7829364       | #B4B4B4    |
| ActiveCaption          | Background of the active window's title bar.                               | (255,153,180,209)    | -10984591      | #99B4D1    |
| ActiveCaptionText      | Text in the active window's title bar.                                     | (255,0,0,0)          | -16777216      | #000000    |
| AppWorkspace           | Application workspace color.                                               | (255,171,171,171)    | -8750475       | #ABABAB    |
| ButtonFace             | Face color of a 3-D element.                                               | (255,240,240,240)    | -3158065       | #F0F0F0    |
| ButtonHighlight        | Highlight color of a 3-D element.                                          | (255,255,255,255)    | -1             | #FFFFFF    |
| ButtonShadow           | Shadow color of a 3-D element.                                             | (255,160,160,160)    | -6250336       | #A0A0A0    |
| Control                | Face color of a 3-D element.                                               | (255,240,240,240)    | -3158065       | #F0F0F0    |
| ControlDark            | Shadow color of a 3-D element.                                             | (255,160,160,160)    | -6250336       | #A0A0A0    |
| ControlDarkDark        | Dark shadow color of a 3-D element.                                        | (255,105,105,105)    | -11184811      | #696969    |
| ControlLight           | Light color of a 3-D element.                                              | (255,227,227,227)    | -5200933       | #E3E3E3    |
| ControlLightLight      | Highlight color of a 3-D element.                                          | (255,255,255,255)    | -1             | #FFFFFF    |
| ControlText            | Text color in a 3-D element.                                               | (255,0,0,0)          | -16777216      | #000000    |
| Desktop                | Background color of the desktop.                                           | (255,0,0,0)          | -16777216      | #000000    |
| GradientActiveCaption  | Lightest color in the gradient of an active window's title bar.            | (255,185,209,234)    | -7323982       | #B9D1EA    |
| GradientInactiveCaption| Lightest color in the gradient of an inactive window's title bar.          | (255,215,228,242)    | -5532806       | #D7E4F2    |
| GrayText               | Color of dimmed text.                                                      | (255,109,109,109)    | -10855845      | #6D6D6D    |
| Highlight              | Background of selected items.                                              | (255,51,153,255)     | -13499137      | #3399FF    |
| HighlightText          | Text of selected items.                                                    | (255,255,255,255)    | -1             | #FFFFFF    |
| HotTrack               | Color used to designate a hot-tracked item.                                | (255,0,102,204)      | -16744708      | #0066CC    |
| InactiveBorder         | Border of an inactive window.                                              | (255,244,247,252)    | -727372        | #F4F7FC    |
| InactiveCaption        | Background of an inactive window's title bar.                              | (255,191,205,219)    | -10855893      | #BFCDDB    |
| InactiveCaptionText    | Text in an inactive window's title bar.                                    | (255,0,0,0)          | -16777216      | #000000    |
| Info                   | Background of a ToolTip.                                                   | (255,255,255,225)    | -31            | #FFFFE1    |
| InfoText               | Text of a ToolTip.                                                         | (255,0,0,0)          | -16777216      | #000000    |
| Menu                   | Background of a menu.                                                      | (255,240,240,240)    | -3158065       | #F0F0F0    |
| MenuBar                | Background of a menu bar.                                                  | (255,240,240,240)    | -3158065       | #F0F0F0    |
| MenuHighlight          | Highlighted menu item.                                                     | (255,51,153,255)     | -13499137      | #3399FF    |
| MenuText               | Text of a menu.                                                            | (255,0,0,0)          | -16777216      | #000000    |
| ScrollBar              | Background of a scroll bar.                                                | (255,200,200,200)    | -3618616       | #C8C8C8    |
| Window                 | Background in the client area of a window.                                 | (255,255,255,255)    | -1             | #FFFFFF    |
| WindowFrame            | Color of a window frame.                                                   | (255,100,100,100)    | -11184811      | #646464    |
| WindowText             | Text in the client area of a window.                                       | (255,0,0,0)          | -16777216      | #000000    |



| .NET Name | Description | ARGB (A,R,G,B) | Integer Value | Hex Code |
| --- | --- | --- | --- |
| ActiveBorder | Color of the active window's border. | | | |
| ActiveCaption | Color of the background of the active window's title bar. | | | |
ActiveCaptionText 	

Gets a Color structure that is the color of the text in the active window's title bar.
AppWorkspace 	

Gets a Color structure that is the color of the application workspace.
ButtonFace 	

Gets a Color structure that is the face color of a 3-D element.
ButtonHighlight 	

Gets a Color structure that is the highlight color of a 3-D element.
ButtonShadow 	

Gets a Color structure that is the shadow color of a 3-D element.
Control 	

Gets a Color structure that is the face color of a 3-D element.
ControlDark 	

Gets a Color structure that is the shadow color of a 3-D element.
ControlDarkDark 	

Gets a Color structure that is the dark shadow color of a 3-D element.
ControlLight 	

Gets a Color structure that is the light color of a 3-D element.
ControlLightLight 	

Gets a Color structure that is the highlight color of a 3-D element.
ControlText 	

Gets a Color structure that is the color of text in a 3-D element.
Desktop 	

Gets a Color structure that is the color of the desktop.
GradientActiveCaption 	

Gets a Color structure that is the lightest color in the color gradient of an active window's title bar.
GradientInactiveCaption 	

Gets a Color structure that is the lightest color in the color gradient of an inactive window's title bar.
GrayText 	

Gets a Color structure that is the color of dimmed text.
Highlight 	

Gets a Color structure that is the color of the background of selected items.
HighlightText 	

Gets a Color structure that is the color of the text of selected items.
HotTrack 	

Gets a Color structure that is the color used to designate a hot-tracked item.
InactiveBorder 	

Gets a Color structure that is the color of an inactive window's border.
InactiveCaption 	

Gets a Color structure that is the color of the background of an inactive window's title bar.
InactiveCaptionText 	

Gets a Color structure that is the color of the text in an inactive window's title bar.
Info 	

Gets a Color structure that is the color of the background of a ToolTip.
InfoText 	

Gets a Color structure that is the color of the text of a ToolTip.
Menu 	

Gets a Color structure that is the color of a menu's background.
MenuBar 	

Gets a Color structure that is the color of the background of a menu bar.
MenuHighlight 	

Gets a Color structure that is the color used to highlight menu items when the menu appears as a flat menu.
MenuText 	

Gets a Color structure that is the color of a menu's text.
ScrollBar 	

Gets a Color structure that is the color of the background of a scroll bar.
UseAlternativeColorSet 	

Gets or sets a value that indicates whether the alternative color set is used.
Window 	

Gets a Color structure that is the color of the background in the client area of a window.
WindowFrame 	

Gets a Color structure that is the color of a window frame.
WindowText 	

Gets a Color structure that is the color of the text in the client area of a window.

SystemColors

| .NET Name | ARGB (A,R,G,B) | Integer Value | Hex Code |
| --- | --- | --- | --- |
| Black | ARGB(255, 0, 0, 0) | -16777216 | #FF000000 | 
| White | ARGB(255, 255, 255, 255) | -1 | #FFFFFFFF | 
| Red | ARGB(255, 255, 0, 0) | -65536 | #FFFF0000 | 
| Green | ARGB(255, 0, 255, 0) | -16711936 | #FF00FF00 | 
| Blue | ARGB(255, 0, 0, 255) | -16776961 | #FF0000FF | 
| Yellow | ARGB(255, 255, 255, 0) | -256 | #FFFFFF00 | 
| Cyan | ARGB(255, 0, 255, 255) | -16711681 | #FF00FFFF | 
| Magenta | ARGB(255, 255, 0, 255) | -65281 | #FFFF00FF | 
| Gray | ARGB(255, 128, 128, 128) | -8355712 | #FF808080 | 
| Orange | ARGB(255, 255, 165, 0) | -23296 | #FFFFA500 | 
| Purple | ARGB(255, 128, 0, 128) | -8388480 | #FF800080 | 
| Brown | ARGB(255, 165, 42, 42) | -5952982 | #FFA52A2A | 
| Lime | ARGB(255, 0, 255, 0) | -16711936 | #FF00FF00 | 
| Navy | ARGB(255, 0, 0, 128) | -16777088 | #FF000080 | 
| Teal | ARGB(255, 0, 128, 128) | -16744320 | #FF008080 | 
| Olive | ARGB(255, 128, 128, 0) | -8355840 | #FF808000 | 
| Silver | ARGB(255, 192, 192, 192) | -4144960 | #FFC0C0C0 | 


| Color Name | Hex Code | Swatch |
|------------|----------|--------|
| Crimson    | `#DC143C`| <span style="display:inline-block;width:20px;height:20px;background-color:#DC143C;border:1px solid #000;"></span> |
| Gold       | `#FFD700`| <span style="display:inline-block;width:20px;height:20px;background-color:#FFD700;border:1px solid #000;"></span> |
| RoyalBlue  | `#4169E1`| <span style="display:inline-block;width:20px;height:20px;background-color:#4169E1;border:1px solid #000;"></span> |
| ForestGreen| `#228B22`| <span style="display:inline-block;width:20px;height:20px;background-color:#228B22;border:1px solid #000;"></span> |




If you're looking for different shades, here are a few more:
| Color Name | ARGB (A,R,G,B) | Hex Value | Integer Value | 
| Light Green | ARGB(255, 144, 238, 144) | 0xFF90EE90 | -11200866 | 
| Dark Green | ARGB(255, 0, 100, 0) | 0xFF006400 | -16751616 | 
| Forest Green | ARGB(255, 34, 139, 34) | 0xFF228B22 | -14498044 | 
| Lime | ARGB(255, 0, 255, 0) | 0xFF00FF00 | -16711936 | 


Absolutely! Here’s a selection of blue shades in ARGB format, just like before:
🔵 Common Blue Shades in ARGB
| Color Name | ARGB (A,R,G,B) | Hex Value | Integer Value | 
| Pure Blue | ARGB(255, 0, 0, 255) | 0xFF0000FF | -16776961 | 
| Light Blue | ARGB(255, 173, 216, 230) | 0xFFADD8E6 | -10988912 | 
| Sky Blue | ARGB(255, 135, 206, 235) | 0xFF87CEEB | -12490271 | 
| Deep Sky Blue | ARGB(255, 0, 191, 255) | 0xFF00BFFF | -16755405 | 
| Dodger Blue | ARGB(255, 30, 144, 255) | 0xFF1E90FF | -14774017 | 
| Navy | ARGB(255, 0, 0, 128) | 0xFF000080 | -16777088 | 
| Midnight Blue | ARGB(255, 25, 25, 112) | 0xFF191970 | -16777216 | 


🧊 Want transparency

